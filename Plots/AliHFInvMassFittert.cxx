...
/// \cond CLASSIMP
ClassImp(AliHFInvMassFittert);
/// \endcond

/////////////////////////////////////////////////////////////
///
/// Implemenatation of AliHFInvMassFittert class for
...
AliHFInvMassFittert::AliHFInvMassFittert() :
  TNamed(),
  fHistoInvMass(0x0),
  fMinMass(0),
  fMaxMass(5),
...
//__________________________________________________________________________
AliHFInvMassFittert::AliHFInvMassFittert(const TH1F *histoToFit, Double_t minvalue, Double_t maxvalue, Int_t fittypeb, Int_t fittypes):
  TNamed(),
  fHistoInvMass(0x0),
  fMinMass(minvalue),
  fMaxMass(maxvalue),
...
//__________________________________________________________________________
void AliHFInvMassFittert::SetNumberOfParams(){
...
//__________________________________________________________________________
Int_t AliHFInvMassFittert::MassFitter(Bool_t draw){
  ...
  fBkgFunc->SetLineColor(kRed+1);
  fBkgFunc->SetLineWidth(2);
  fBkgFunc->SetLineStyle(7);
  ...
  if(doFinalFit){
    ...
    printf("[AliHFInvMassFittert] final fit status %d\n",status);
    printf("[AliHFInvMassFittert] IsValid() = %d\n",isFitValid);
    if ( (status!=0 && !fAcceptValidFit) || (fAcceptValidFit && !isFitValid) ){
...
void AliHFInvMassFittert::DrawFit(){
...
//______________________________________________________________________________
void AliHFInvMassFittert::DrawHere(TVirtualPad* c, Double_t nsigma,Int_t writeFitInfo){
  /// Core method to draw the fit output
  ///

  TLegend * leg = new TLegend(0.6, 0.5, 0.84, 0.7);
...
//  fHistoInvMass->GetXaxis()->SetRangeUser(fMinMass,fMaxMass);
  fHistoInvMass->GetXaxis()->SetRangeUser(1.72, 2.03);
  //fHistoInvMass->GetYaxis()->SetRangeUser(0,6000); D0 0124
  //fHistoInvMass->GetYaxis()->SetRangeUser(0,40000); Dplus 0124
  fHistoInvMass->GetYaxis()->SetRangeUser(0.01,800); //Dplus MB

  fHistoInvMass->GetYaxis()->SetNdivisions(505);
  fHistoInvMass->GetYaxis()->SetTitleOffset(1.3);

  fHistoInvMass->SetMarkerStyle(20);
  fHistoInvMass->SetMinimum(0.01);
  ...
  leg->AddEntry(fTotFunc, "Total fit function", "l");
  leg->AddEntry(fBkgFunc, "Combinatorial ", "l");
  leg->AddEntry(fRflFunc, "K#minus#pi reflected", "l");
  ...
  TLatex* preinfo = new TLatex(8, 8, "");
  preinfo->SetNDC();
  preinfo->SetTextFont(42);
  preinfo->SetTextSize(0.06);
//preinfo->SetBorderSize(0);
//preinfo->SetFillStyle(0);
  preinfo->SetTextColor(kBlack);
  preinfo->DrawLatex(0.15, 0.87, "ALICE Preliminary");
  preinfo->SetTextSize(0.05);
  preinfo->DrawLatex(0.15, 0.81,"p#font[122]{-}Pb, #sqrt{#it{s}_{NN}} = 5.02 TeV");
  //preinfo->DrawLatex(0.17, 0.82,"pp, #sqrt{#it{s}} = 13 TeV, High Multiplicity");

  preinfo->SetTextSize(0.04);
  preinfo->DrawLatex(0.15, 0.74, "D^{0}#rightarrow K^{#minus}#pi^{+} and charge conj.");
//  preinfo->DrawLatex(0.17, 0.75, "D^{#plus}#rightarrow K^{#minus}#pi^{+}#pi^{+} and charge conj.");

  //preinfo->DrawLatex(0.18, 0.66, "1 < #it{p}_{T} < 24 GeV/#it{c}");
  preinfo->DrawLatex(0.15, 0.68, "3 < #it{p}_{T} < 4 GeV/#it{c}");
  preinfo->SetTextSize(0.04);
  preinfo->DrawLatex(0.15, 0.61,Form("#mu = (%.0f #pm %.0f) MeV/#it{c}^{2}",1867.,1.));
  preinfo->DrawLatex(0.15, 0.56,Form("#sigma = %.0f MeV/#it{c}^{2}",10.));
  preinfo->DrawLatex(0.15, 0.51,Form("S = %.0f #pm %.0f ",fRawYield,fRawYieldErr));
  
  preinfo->SetTextSize(0.04);
  //preinfo->DrawLatex(0.15, 0.15, "#it{f}_{non-pompt} = 0.223 #pm 0.012 (stat.) #pm 0.011 (syst.)");
  //preinfo->DrawLatex(0.15, 0.15, "#it{f}_{non-pompt} = 0.261 #pm 0.007 (stat.) #pm 0.008 (syst.)");
  //preinfo->DrawLatex(0.15, 0.15, "#it{f}_{non-pompt} = 0.344 #pm 0.010 (stat.) #pm 0.010 (syst.)");
  //preinfo->DrawLatex(0.15, 0.15, "#it{f}_{non-pompt} = 0.296 #pm 0.012 (stat.) #pm 0.009 (syst.)");
  preinfo->DrawLatex(0.15, 0.15, "#it{f}^{ raw}_{non-prompt} = 0.750 #pm 0.016 (stat.) #pm 0.0075 (syst.)");

  c->Update();
  return;
}

void AliHFInvMassFittert::DrawHere1(TVirtualPad* c, Double_t nsigma,Int_t writeFitInfo)
{
  /// Core method to draw the fit output
  ///

  TLegend * leg = new TLegend(0.18, 0.7, 0.44, 0.87);
  ...
  c->cd();
  fHistoInvMass->GetXaxis()->SetRangeUser(fMinMass,fMaxMass);
  //fHistoInvMass->GetYaxis()->SetRangeUser(0,1600);// D0 0124
  //fHistoInvMass->GetYaxis()->SetRangeUser(0,9000);// Dplus 0124
  fHistoInvMass->GetYaxis()->SetRangeUser(0,6000);// Dplus MB

  fHistoInvMass->GetYaxis()->SetNdivisions(505);

  //fHistoInvMass->GetYaxis()->SetNoExponent();
  fHistoInvMass->SetMarkerStyle(20);
  ...
  leg->AddEntry(fTotFunc, "Total fit function", "l");
  leg->AddEntry(fBkgFunc, "Combinatorial background", "l");
  leg->AddEntry(fRflFunc, "K#minus#pi reflected", "l");
...
//preinfo->SetFillStyle(0);
  preinfo->SetTextColor(kBlack);
  //preinfo->DrawLatex(0.15, 0.15, "#it{f}_{non-pompt} = 0.951 #pm 0.003 (stat.) #pm 0.047 (syst.)");
  //preinfo->DrawLatex(0.15, 0.15, "#it{f}_{non-pompt} = 0.855 #pm 0.006 (stat.) #pm 0.025 (syst.)");
  //preinfo->DrawLatex(0.15, 0.15, "#it{f}_{non-pompt} = 0.983 #pm 0.001 (stat.) #pm 0.030 (syst.)");
 // preinfo->DrawLatex(0.15, 0.15, "#it{f}_{non-pompt} = 0.847 #pm 0.007 (stat.) #pm 0.028 (syst.)");
   preinfo->DrawLatex(0.15, 0.15, "#it{f}_{non-pompt} = 0.911 #pm 0.009 (stat.) #pm 0.036 (syst.)");

  
  preinfo->SetTextSize(0.05);

  preinfo->DrawLatex(0.17, 0.62,Form("S = %.0f #pm %.0f ",fRawYield,fRawYieldErr));
...
//______________________________________________________________________________
void AliHFInvMassFittert::DrawHistoMinusFit(TVirtualPad* c, Int_t writeFitInfo){
  /// Core method to draw the fit output
  ///
...
//______________________________________________________________________________
Double_t AliHFInvMassFittert::CheckForSignal(Double_t mean, Double_t sigma){
...
//______________________________________________________________________________
TF1* AliHFInvMassFittert::CreateBackgroundFitFunction(TString fname, Double_t integral){
  /// Creates the background fit fucntion
  ///

  SetNumberOfParams();
  TF1* funcbkg =  new TF1(fname.Data(),this,&AliHFInvMassFittert::FitFunction4Bkg,fMinMass,fMaxMass,fNParsBkg,"AliHFInvMassFittert","FitFunction4Bkg");
  switch (fTypeOfFit4Bkg) {
  case 0: //gaus+expo
...
//______________________________________________________________________________
TF1* AliHFInvMassFittert::CreateSecondPeakFunction(TString fname, Double_t integsig){
  ...
  TF1* funcsec =  new TF1(fname.Data(),this,&AliHFInvMassFittert::FitFunction4SecPeak,fMinMass,fMaxMass,3,"AliHFInvMassFittert","FitFunction4SecPeak");
...
//______________________________________________________________________________
TF1* AliHFInvMassFittert::CreateReflectionFunction(TString fname){
  /// Creates a function for reflections contribution
  /// in the D0->Kpi inv. mass distribution
  TF1* funcrfl =  new TF1(fname.Data(),this,&AliHFInvMassFittert::FitFunction4Refl,fMinMass,fMaxMass,1,"AliHFInvMassFittert","FitFunction4Refl");
...
//______________________________________________________________________________
TF1* AliHFInvMassFittert::CreateBackgroundPlusReflectionFunction(TString fname){
  /// Creates the function with sum of background and reflections
  ///
  SetNumberOfParams();
  Int_t totParams=fNParsBkg+fNParsRfl;
  TF1* fbr=new TF1(fname.Data(),this,&AliHFInvMassFittert::FitFunction4BkgAndRefl,fMinMass,fMaxMass,totParams,"AliHFInvMassFittert","FitFunction4BkgAndRefl");
  for(Int_t ipar=0; ipar<fNParsBkg; ipar++){
...
//______________________________________________________________________________
TF1* AliHFInvMassFittert::CreateSignalFitFunction(TString fname, Double_t integsig){
  /// Creates the fit function for the signal peak
  ///

  SetNumberOfParams();
  TF1* funcsig =  new TF1(fname.Data(),this,&AliHFInvMassFittert::FitFunction4Sgn,fMinMass,fMaxMass,fNParsSig,"AliHFInvMassFittert","FitFunction4Sgn");
...
//______________________________________________________________________________
TF1* AliHFInvMassFittert::CreateTotalFitFunction(TString fname){
  ...
  TF1* ftot=new TF1(fname.Data(),this,&AliHFInvMassFittert::FitFunction4Mass,fMinMass,fMaxMass,totParams,"AliHFInvMassFittert","FitFunction4Mass");
  for(Int_t ipar=0; ipar<fNParsBkg; ipar++){
...
//__________________________________________________________________________
Double_t AliHFInvMassFittert::FitFunction4Bkg (Double_t *x, Double_t *par){
...
//_________________________________________________________________________
Double_t AliHFInvMassFittert::FitFunction4Sgn (Double_t *x, Double_t *par){
...
//_________________________________________________________________________
Double_t AliHFInvMassFittert::FitFunction4Refl(Double_t *x,Double_t *par){
...
//_________________________________________________________________________
Double_t AliHFInvMassFittert::FitFunction4BkgAndRefl(Double_t *x, Double_t *par){
...
//_________________________________________________________________________
Double_t AliHFInvMassFittert::FitFunction4SecPeak (Double_t *x, Double_t *par){
...
//_________________________________________________________________________
Double_t AliHFInvMassFittert::FitFunction4Mass(Double_t *x, Double_t *par){
...
//_________________________________________________________________________
void AliHFInvMassFittert::Signal(Double_t nOfSigma,Double_t &signal,Double_t &errsignal) const {
...
//_________________________________________________________________________
void AliHFInvMassFittert::Signal(Double_t min, Double_t max, Double_t &signal,Double_t &errsignal) const {
...
void AliHFInvMassFittert::Background(Double_t nOfSigma,Double_t &background,Double_t &errbackground) const {
...
//___________________________________________________________________________
void AliHFInvMassFittert::Background(Double_t min, Double_t max, Double_t &background,Double_t &errbackground) const {
...
void AliHFInvMassFittert::Significance(Double_t nOfSigma,Double_t &significance,Double_t &errsignificance) const  {
...
void AliHFInvMassFittert::Significance(Double_t min, Double_t max, Double_t &significance,Double_t &errsignificance) const {
...
Bool_t AliHFInvMassFittert::PrepareHighPolFit(TF1 *fback){
  ...
  while(fCurPolDegreeBkg<=fPolDegreeBkg){
    funcbkg = new TF1(Form("temp%d",fCurPolDegreeBkg),this,&AliHFInvMassFittert::BackFitFuncPolHelper,fMinMass,fMaxMass,fCurPolDegreeBkg+1,"AliHFInvMassFittert","BackFitFuncPolHelper");
    if(funcPrev){
...
Double_t AliHFInvMassFittert::BackFitFuncPolHelper(Double_t *x,Double_t *par){
...
// _______________________________________________________________________
TH1F* AliHFInvMassFittert::SetTemplateReflections(const TH1 *h, TString opt,Double_t minRange,Double_t maxRange){
...
// _______________________________________________________________________
Double_t AliHFInvMassFittert::GetRawYieldBinCounting(Double_t& errRyBC, Double_t nOfSigma, Int_t option, Int_t pdgCode) const{
...
// _______________________________________________________________________
Double_t AliHFInvMassFittert::GetRawYieldBinCounting(Double_t& errRyBC, Double_t minMass, Double_t maxMass, Int_t option) const{
  /// Method to compute the signal using inv. mass histo bin counting
...
// _______________________________________________________________________
TH1F* AliHFInvMassFittert::GetResidualsAndPulls(TH1 *hPulls,TH1 *hResidualTrend,TH1 *hPullsTrend, Double_t minrange,Double_t maxrange, Int_t option){
...
// _______________________________________________________________________
TH1F* AliHFInvMassFittert::GetOverBackgroundResidualsAndPulls(TH1 *hPulls,TH1 *hResidualTrend,TH1 *hPullsTrend, Double_t minrange,Double_t maxrange){
  ///
  return GetResidualsAndPulls(hPulls,hResidualTrend,hPullsTrend,minrange,maxrange,1);
}
// _______________________________________________________________________
TH1F* AliHFInvMassFittert::GetOverBackgroundPlusReflResidualsAndPulls(TH1 *hPulls,TH1 *hResidualTrend,TH1 *hPullsTrend, Double_t minrange,Double_t maxrange){
  ///
  return GetResidualsAndPulls(hPulls,hResidualTrend,hPullsTrend,minrange,maxrange,2);
}

// _______________________________________________________________________
void AliHFInvMassFittert::PrintFunctions(){
...
TH1D* AliHFInvMassFittert::RebinHisto(TH1* hOrig, Int_t reb, Int_t firstUse){
...
void AliHFInvMassFittert::ComputeSignificance(Double_t signal, Double_t  errsignal, Double_t  background, Double_t  errbackground, Double_t &significance,Double_t &errsignificance){
