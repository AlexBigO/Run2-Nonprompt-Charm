...
// Macro for fitting D+, D0 and Ds+ invariant-mass spectra // Main Function:
// GetRawYieldsDplusDs_plot                                                //
//___________________________________________________________________________________//
...
int GetRawYieldsDplusDs_plot(
    int cent = kpPb5TeVFD, bool isMC = false,
    TString infilename1 = "./ProjectedSparseData_D0_pPb5TeV_FD.root",
    TString refFileName1 = "./ProjectedSparseMC_D0_pPb5TeV_FD.root",
    TString infilename2 = "./ProjectedSparseData_D0_pPb5TeV_FD.root",
    TString refFileName2 = "./ProjectedSparseMC_D0_pPb5TeV_FD.root",
    TString cfgfilename = "./config_D0_Fit_pPb5TeV_Data.yml",
    TString outFileName = "./RawYieldPlot.root");
// GetRawYieldsDplusDs_plot(kpPb5TeVFD,
// false,"./RawYieldsData_D0_pPb5TeV_FD.root",
// "./RawYieldsMC_D0_pPb5TeV_FD.root",
// "./RawYieldsData_D0_pPb5TeV_FD_pos5.root",
// "./RawYieldsMC_D0_pPb5TeV_FD_pos5.root","./config_D0_Fit_pPb5TeV_Data.yml",
// "./RawYieldPlot.root")
...
//__________________________________________________________________________________________________________________
int GetRawYieldsDplusDs_plot(int cent, bool isMC, TString infilename1,
                             TString refFileName1, TString infilename2,
                             TString refFileName2, TString cfgfilename,
                             TString outFileName) {
  SetStyle();
  ...
  if (cent == k010)
    ...
    centname = "Cent6080";
  else if (cent == kpp5TeVPrompt)
    centname = "pp5TeVPrompt";
  else if (cent == kpp5TeVFD)
    centname = "pp5TeVFD";
  else if (cent == kpp13TeVPrompt)
    centname = "pp13TeVPrompt";
  else if (cent == kpp13TeVFD)
    centname = "pp13TeVFD";
  else if (cent == kpPb5TeVFD)
    centname = "pPb5TeVFD";
  else {
  ...
  vector<double> MassMin =
      config[centname.Data()]["MassMin"].as<vector<double>>();
  vector<double> MassMax =
      config[centname.Data()]["MassMax"].as<vector<double>>();
  vector<int> fixSigma = {};
  cerr << "ERROR: 3333" << endl;
  ...
  string infilenameSigmaRatio =
      config[centname.Data()]["SigmaRatioFile"].as<string>();
  ...
  vector<int> InclSecPeak =
      config[centname.Data()]["InclSecPeak"].as<vector<int>>();
  vector<double> SigmaSecPeak =
      config[centname.Data()]["SigmaSecPeak"].as<vector<double>>();
  string infilenameSigmaSecPeak =
      config[centname.Data()]["SigmaFileSecPeak"].as<string>();
  double sigmaMultSecPeak =
      config[centname.Data()]["SigmaMultFactorSecPeak"].as<double>();
  bool fixSigmaToFirstPeak = static_cast<bool>(
      config[centname.Data()]["FixSigmaToFirstPeak"].as<int>());
  vector<string> bkgfunc =
      config[centname.Data()]["BkgFunc"].as<vector<string>>();
  vector<string> sgnfunc =
      config[centname.Data()]["SgnFunc"].as<vector<string>>();
  const unsigned int nPtBins = PtMin.size();
  int BkgFunc[nPtBins], SgnFunc[nPtBins], degPol[nPtBins];
  double PtLims[nPtBins + 1];
  cerr << "ERROR: 5555" << endl;
  ...
  for (unsigned int iPt = 0; iPt < nPtBins; iPt++) {
    ...
    if (bkgfunc[iPt] == "kExpo")
      BkgFunc[iPt] = AliHFInvMassFittert::kExpo;
    else if (bkgfunc[iPt] == "kLin")
      BkgFunc[iPt] = AliHFInvMassFittert::kLin;
    else if (bkgfunc[iPt] == "kPol2")
      BkgFunc[iPt] = AliHFInvMassFittert::kPol2;
    else if (bkgfunc[iPt] == "kPol3") {
      BkgFunc[iPt] = 6;
      degPol[iPt] = 3;
      if (PtMin.size() > 1 && InclSecPeak[iPt] == 1) {
        cerr << "Pol3 and Pol4 fits work only with one bin if you have the "
                "secondary peak! Exit!"
             << endl;
        return -1;
      }
      ...
      BkgFunc[iPt] = AliHFInvMassFittert::kPow;
    else if (bkgfunc[iPt] == "kPowEx")
      BkgFunc[iPt] = AliHFInvMassFittert::kPowEx;
    else {
      cerr << "ERROR: only kExpo, kLin, kPol2, kPol3, and kPol4, kPow and "
              "kPowEx background functions supported! Exit"
           << endl;
      return -1;
    }

    cerr << "ERROR: 6666" << endl;

    if (sgnfunc[iPt] == "kGaus")
      SgnFunc[iPt] = AliHFInvMassFittert::kGaus;
    else if (sgnfunc[iPt] == "k2Gaus")
      SgnFunc[iPt] = AliHFInvMassFittert::k2Gaus;
    else if (sgnfunc[iPt] == "k2GausSigmaRatioPar")
      SgnFunc[iPt] = AliHFInvMassFittert::k2GausSigmaRatioPar;
    else {
      cerr << "ERROR: only kGaus, k2Gaus and k2GausSigmaRatioPar signal "
              "functions supported! Exit"
           << endl;
      return -1;
    }
  }
  cerr << "ERROR: 7777" << endl;
  ...
  else if (particle == kLc)
    massaxistit = "#it{M}(pK^{0}_{s}) (GeV/#it{c}^{2})";
  else if (particle == kDstar)
    massaxistit = "#it{M}(pi^{+}) (GeV/#it{c}^{2})";

  // load inv-mass histos
  auto infile1 = TFile::Open(infilename1.Data());
  auto infile2 = TFile::Open(infilename2.Data());

  if (!infile1 || !infile1->IsOpen())
    return -1;
  if (!infile2 || !infile2->IsOpen())
    return -1;

  TFile *infileref1 = NULL;
  TFile *infileref2 = NULL;
  if (enableRef) {
    infileref1 = TFile::Open(refFileName1.Data());
    infileref2 = TFile::Open(refFileName2.Data());
    if (!infileref1 || !infileref1->IsOpen())
      return -1;
    if (!infileref2 || !infileref2->IsOpen())
      return -1;
  }
  cerr << "ERROR: 8888" << endl;

  TH1F *hMassSig1[nPtBins];
  TH1F *hMassSig2[nPtBins];

  TH1F *hMassRef1[nPtBins];
  TH1F *hMassRef2[nPtBins];

  TH1F *hMass1[nPtBins];
  TH1F *hMass2[nPtBins];

  TH1F *hEv = NULL;
  cerr << "ERROR: 9999" << endl;

  for (unsigned int iPt = 0; iPt < nPtBins; iPt++) {

    if (!isMC) {

      hMass1[iPt] = static_cast<TH1F *>(infile1->Get(
          Form("hMass_%0.f_%0.f", PtMin[iPt] * 10, PtMax[iPt] * 10)));
      hMass2[iPt] = static_cast<TH1F *>(infile2->Get(
          Form("hMass_%0.f_%0.f", PtMin[iPt] * 10, PtMax[iPt] * 10)));

      if (enableRef) {
        hMassRef1[iPt] = static_cast<TH1F *>(infileref1->Get(
            Form("hVarReflMass_%0.f_%0.f", PtMin[iPt] * 10, PtMax[iPt] * 10)));
        hMassRef2[iPt] = static_cast<TH1F *>(infileref2->Get(
            Form("hVarReflMass_%0.f_%0.f", PtMin[iPt] * 10, PtMax[iPt] * 10)));
        hMassSig1[iPt] = static_cast<TH1F *>(infileref1->Get(
            Form("hFDMass_%0.f_%0.f", PtMin[iPt] * 10, PtMax[iPt] * 10)));
        hMassSig2[iPt] = static_cast<TH1F *>(infileref2->Get(
            Form("hFDMass_%0.f_%0.f", PtMin[iPt] * 10, PtMax[iPt] * 10)));
        hMassSig1[iPt]->Add(static_cast<TH1F *>(infileref1->Get(
            Form("hPromptMass_%0.f_%0.f", PtMin[iPt] * 10, PtMax[iPt] * 10))));
        hMassSig2[iPt]->Add(static_cast<TH1F *>(infileref2->Get(
            Form("hPromptMass_%0.f_%0.f", PtMin[iPt] * 10, PtMax[iPt] * 10))));
      }
      ...
      hMass1[iPt]->Add(static_cast<TH1F *>(infile1->Get(
          Form("hFDMass_%0.f_%0.f", PtMin[iPt] * 10, PtMax[iPt] * 10))));
      hMass2[iPt]->Add(static_cast<TH1F *>(infile2->Get(
          Form("hFDMass_%0.f_%0.f", PtMin[iPt] * 10, PtMax[iPt] * 10))));

      if (InclSecPeak[iPt]) {
        hMass1[iPt]->Add(static_cast<TH1F *>(
            infile1->Get(Form("hPromptSecPeakMass_%0.f_%0.f", PtMin[iPt] * 10,
                              PtMax[iPt] * 10))));
        hMass1[iPt]->Add(static_cast<TH1F *>(infile1->Get(Form(
            "hFDSecPeakMass_%0.f_%0.f", PtMin[iPt] * 10, PtMax[iPt] * 10))));
      }
    }
    cerr << "ERROR: 99994" << endl;

    // hEv = static_cast<TH1F*>(infile->Get("hEvForNorm"));
    hMass1[iPt]->SetDirectory(0);
    hMass2[iPt]->SetDirectory(0);

    // hEv->SetDirectory(0);
    cerr << "ERROR: 99995" << endl;

    SetHistoStyle(hMass1[iPt]);
    SetHistoStyle(hMass2[iPt]);
  ...
  auto hRawYieldsSigmaRatio = new TH1D(
      ...
      ";#it{p}_{T} (GeV/#it{c});ratio #sigma_{1}/#sigma_{2}", nPtBins, PtLims);
  ...
  auto hRawYieldsFracGaus2 = new TH1D(
      "hRawYieldsFracGaus2",
      ";#it{p}_{T} (GeV/#it{c});second-gaussian fraction", nPtBins, PtLims);
  auto hRawYieldsSignificance = new TH1D(
      "hRawYieldsSignificance",
      ";#it{p}_{T} (GeV/#it{c});significance (3#sigma)", nPtBins, PtLims);
  auto hRawYieldsSoverB =
      new TH1D("hRawYieldsSoverB", ";#it{p}_{T} (GeV/#it{c});S/B (3#sigma)",
               nPtBins, PtLims);
  ...
  auto hRawYieldsChiSquare =
      new TH1D("hRawYieldsChiSquare",
               ";#it{p}_{T} (GeV/#it{c});#chi^{2}/#it{ndf}", nPtBins, PtLims);
  ...
  auto hRawYieldsSoverBSecondPeak = new TH1D(
      ...
      ";#it{p}_{T} (GeV/#it{c});S/B second peak (3#sigma)", nPtBins, PtLims);
  auto hRawYieldsSignalSecondPeak = new TH1D(
      "hRawYieldsSignalSecondPeak",
      ";#it{p}_{T} (GeV/#it{c});Signal second peak (3#sigma)", nPtBins, PtLims);
  auto hRawYieldsBkgSecondPeak =
      new TH1D("hRawYieldsBkgSecondPeak",
               ";#it{p}_{T} (GeV/#it{c});Background second peak (3#sigma)",
               nPtBins, PtLims);
  auto hRawYieldsTrue =
      new TH1D("hRawYieldsTrue", ";#it{p}_{T} (GeV/#it{c});true signal",
               nPtBins, PtLims);
  auto hRawYieldsSecondPeakTrue = new TH1D(
      "hRawYieldsSecondPeakTrue",
      ";#it{p}_{T} (GeV/#it{c});true signal second peak", nPtBins, PtLims);
  auto hRelDiffRawYieldsFitTrue =
      new TH1D("hRelDiffRawYieldsFitTrue",
               ";#it{p}_{T} (GeV/#it{c}); (Y_{fit} - Y_{true}) / Y_{true}",
               nPtBins, PtLims);
  auto hRelDiffRawYieldsSecondPeakFitTrue = new TH1D(
      "hRelDiffRawYieldsSecondPeakFitTrue",
      ";#it{p}_{T} (GeV/#it{c});(Y_{fit} - Y_{true}) / Y_{true} second peak",
      nPtBins, PtLims);
  ...
  // additional S, B, S/B, and significance histos for different Nsigma values
  // (filled only in case of data)
  const int nMassWindows = 6;
  double nSigma4SandB[nMassWindows] = {1.5, 1.75, 2., 2.25, 2.5, 2.75};
  TH1D *hRawYieldsSignalDiffSigma[nMassWindows],
      *hRawYieldsBkgDiffSigma[nMassWindows];
  TH1D *hRawYieldsSoverBDiffSigma[nMassWindows],
      *hRawYieldsSignifDiffSigma[nMassWindows];
  for (int iS = 0; iS < nMassWindows; iS++) {
    hRawYieldsSignalDiffSigma[iS] = new TH1D(
        Form("hRawYieldsSignal_%0.2fsigma", nSigma4SandB[iS]),
        Form(";#it{p}_{T} (GeV/#it{c});Signal (%0.2f#sigma)", nSigma4SandB[iS]),
        nPtBins, PtLims);
    hRawYieldsBkgDiffSigma[iS] =
        new TH1D(Form("hRawYieldsBkg_%0.2fsigma", nSigma4SandB[iS]),
                 Form(";#it{p}_{T} (GeV/#it{c});Background (%0.2f#sigma)",
                      nSigma4SandB[iS]),
                 nPtBins, PtLims);
    hRawYieldsSoverBDiffSigma[iS] = new TH1D(
        Form("hRawYieldsSoverB_%0.2fsigma", nSigma4SandB[iS]),
        Form(";#it{p}_{T} (GeV/#it{c});S/B (%0.2f#sigma)", nSigma4SandB[iS]),
        nPtBins, PtLims);
    hRawYieldsSignifDiffSigma[iS] =
        new TH1D(Form("hRawYieldsSignif_%0.2fsigma", nSigma4SandB[iS]),
                 Form(";#it{p}_{T} (GeV/#it{c});significance (%0.2f#sigma)",
                      nSigma4SandB[iS]),
                 nPtBins, PtLims);
  ...
  if (accumulate(fixSigma.begin(), fixSigma.end(), 0) > 0) {
    ...
    if (static_cast<unsigned int>(hSigmaToFix->GetNbinsX()) != nPtBins)
      cout << "WARNING: Different number of bins for this analysis and histo "
              "for fix sigma"
           << endl;
    infileSigma->Close();
  }
  ...
  if (fixSigmaRatio) {
    std::cout << "Load sigma ratio from file " << infilenameSigmaRatio.data()
              << std::endl;
    auto infileSigmaRatio = TFile::Open(infilenameSigmaRatio.data());
    if (!infileSigmaRatio)
      return -2;
    hSigmaToFix1 =
        static_cast<TH1D *>(infileSigmaRatio->Get("hRawYieldsSigma"));
    ...
    hSigmaToFix2 =
        static_cast<TH1D *>(infileSigmaRatio->Get("hRawYieldsSigma2"));
  ...
  if (infileSigmaSecPeak) {
    hSigmaFirstPeakMC =
        static_cast<TH1D *>(infileSigmaSecPeak->Get("hRawYieldsSigma"));
    hSigmaToFixSecPeak = static_cast<TH1D *>(
        infileSigmaSecPeak->Get("hRawYieldsSigmaSecondPeak"));
    ...
    if (static_cast<unsigned int>(hSigmaFirstPeakMC->GetNbinsX()) != nPtBins ||
      ...
      cout << "WARNING: Different number of bins for this analysis and histos "
              "for fix sigma"
           << endl;
  ...
  double dmassDstar = TDatabasePDG::Instance()->GetParticle(413)->Mass() -
  ...
  TH1F *hMassForFit1[nPtBins];
  TH1F *hMassForFit2[nPtBins];
  TH1F *hMassForRel1[nPtBins];
  TH1F *hMassForRel2[nPtBins];
  TH1F *hMassForSig1[nPtBins];
  TH1F *hMassForSig2[nPtBins];
  ...
  int nMaxCanvases =
      20; // do not put more than 20 bins per canvas to make them visible
  ...
  for (int iCanv = 0; iCanv < nCanvases; iCanv++) {
    ...
    cMass[iCanv] = new TCanvas(Form("cMass%d", iCanv), Form("cMass%d", iCanv),
                               canvSize[0], canvSize[1]);
    DivideCanvas(cMass[iCanv], nPads);
    cResiduals[iCanv] =
        new TCanvas(Form("cResiduals%d", iCanv), Form("cResiduals%d", iCanv),
                    canvSize[0], canvSize[1]);
  ...
  for (unsigned int iPt = 0; iPt < nPtBins; iPt++) {
    ...
    hMassForFit1[iPt] = reinterpret_cast<TH1F *>(
        AliHFInvMassFittert::RebinHisto(hMass1[iPt], Rebin[iPt]));
    hMassForFit2[iPt] = reinterpret_cast<TH1F *>(
        AliHFInvMassFittert::RebinHisto(hMass2[iPt], Rebin[iPt]));

    TString pttitle =
        Form("%0.1f < #it{p}_{T} < %0.1f GeV/#it{c}", PtMin[iPt], PtMax[iPt]);
    hMassForFit1[iPt]->SetTitle(Form(";%s;Counts per %0.f MeV/#it{c}^{2}",
                                     massaxistit.Data(),
                                     hMassForFit1[iPt]->GetBinWidth(1) * 1000));
    hMassForFit2[iPt]->SetTitle(Form(";%s;Counts per %0.f MeV/#it{c}^{2}",
                                     massaxistit.Data(),
                                     hMassForFit2[iPt]->GetBinWidth(1) * 1000));
    hMassForFit1[iPt]->SetName(Form("MassForFit%d", iPt));
    hMassForFit2[iPt]->SetName(Form("MassForFit%d", iPt));

    hMassForFit1[iPt]->GetXaxis()->SetTitleSize(0.045);
    hMassForFit2[iPt]->GetXaxis()->SetTitleSize(0.045);
    hMassForFit1[iPt]->GetXaxis()->SetLabelSize(0.045);
    hMassForFit2[iPt]->GetXaxis()->SetLabelSize(0.045);
    hMassForFit1[iPt]->GetXaxis()->SetTitleOffset(1.1);
    hMassForFit2[iPt]->GetXaxis()->SetTitleOffset(1.1);
    hMassForFit1[iPt]->GetYaxis()->SetTitleSize(0.045);
    hMassForFit2[iPt]->GetYaxis()->SetTitleSize(0.045);
    hMassForFit1[iPt]->GetYaxis()->SetLabelSize(0.045);
    hMassForFit2[iPt]->GetYaxis()->SetLabelSize(0.045);
    hMassForFit1[iPt]->GetYaxis()->SetTitleOffset(1.1);
    hMassForFit2[iPt]->GetYaxis()->SetTitleOffset(1.1);

    if (enableRef) {
      hMassForRel1[iPt] = reinterpret_cast<TH1F *>(
          AliHFInvMassFittert::RebinHisto(hMassRef1[iPt], Rebin[iPt]));
      hMassForSig1[iPt] = reinterpret_cast<TH1F *>(
          AliHFInvMassFittert::RebinHisto(hMassSig1[iPt], Rebin[iPt]));

      hMassForRel2[iPt] = reinterpret_cast<TH1F *>(
          AliHFInvMassFittert::RebinHisto(hMassRef2[iPt], Rebin[iPt]));
      hMassForSig2[iPt] = reinterpret_cast<TH1F *>(
          AliHFInvMassFittert::RebinHisto(hMassSig2[iPt], Rebin[iPt]));
    }
    double markerSize = 1.;
    if (nPtBins > 15)
      markerSize = 0.5;
    SetHistoStyle(hMassForFit1[iPt], kBlack, markerSize);

    if (isMC) { // MC
      int parRawYield = 0, parMean = 1., parSigma1 = 2,
          parSigmaRatio = 3; // always the same
      int parSigma2 = -1, parFrac2Gaus = -1, parRawYieldSecPeak = -1,
          parMeanSecPeak = -1, parSigmaSecPeak = -1;
      TF1 *massFunc = NULL;
      if (SgnFunc[iPt] == AliHFInvMassFittert::kGaus) {
        if (!(InclSecPeak[iPt] && particle == kDs)) {
          massFunc = new TF1(Form("massFunc%d", iPt), SingleGaus, MassMin[iPt],
                             MassMax[iPt], 3);
          massFunc->SetParameters(hMassForFit1[iPt]->Integral() *
                                      hMassForFit1[iPt]->GetBinWidth(1),
                                  massForFit, 0.010);
        } else {
          massFunc = new TF1(Form("massFunc%d", iPt), DoublePeakSingleGaus,
                             MassMin[iPt], MassMax[iPt], 6);
          massFunc->SetParameters(
              hMassForFit1[iPt]->Integral() * hMassForFit1[iPt]->GetBinWidth(1),
              massForFit, 0.010,
              hMassForFit1[iPt]->Integral() * hMassForFit1[iPt]->GetBinWidth(1),
              massDplus, 0.010);
      ...
      } else if (SgnFunc[iPt] == AliHFInvMassFittert::k2Gaus) {
        parSigma2 = 3;
        parFrac2Gaus = 4;
        if (!(InclSecPeak[iPt] && particle == kDs)) {
          massFunc = new TF1(Form("massFunc%d", iPt), DoubleGaus, MassMin[iPt],
                             MassMax[iPt], 5);
          massFunc->SetParameters(hMassForFit1[iPt]->Integral() *
                                      hMassForFit1[iPt]->GetBinWidth(1),
                                  massForFit, 0.010, 0.030, 0.9);
        } else {
          massFunc = new TF1(Form("massFunc%d", iPt), DoublePeakDoubleGaus,
                             MassMin[iPt], MassMax[iPt], 8);
          massFunc->SetParameters(
              hMassForFit1[iPt]->Integral() * hMassForFit1[iPt]->GetBinWidth(1),
              massForFit, 0.010, 0.030, 0.9,
              hMassForFit1[iPt]->Integral() * hMassForFit1[iPt]->GetBinWidth(1),
      ...
      } else if (SgnFunc[iPt] == AliHFInvMassFittert::k2GausSigmaRatioPar) {
        massFunc = new TF1(Form("massFunc%d", iPt), DoubleGausSigmaRatio,
                           MassMin[iPt], MassMax[iPt], 5);
        massFunc->SetParameters(hMassForFit1[iPt]->Integral() *
                                    hMassForFit1[iPt]->GetBinWidth(1),
      ...
      if (nPtBins > 1)
        ...
        cMass[iCanv]->cd();
      hMassForFit1[iPt]->Fit(massFunc, "E"); // fit with chi2
      ...
      hRawYieldsTrue->SetBinContent(iPt + 1, hMassForFit1[iPt]->Integral());
      hRawYieldsTrue->SetBinError(iPt + 1,
                                  TMath::Sqrt(hMassForFit1[iPt]->Integral()));
      hRelDiffRawYieldsFitTrue->SetBinContent(
          iPt + 1, rawyield - hMassForFit1[iPt]->Integral());
      hRelDiffRawYieldsFitTrue->SetBinError(
          iPt + 1, TMath::Sqrt(rawyielderr * rawyielderr +
                               hMassForFit1[iPt]->Integral()));
      ...
      if (InclSecPeak[iPt] && particle == kDs) {
        ...
        hRawYieldsSigmaRatioSecondFirstPeak->SetBinContent(
            iPt + 1, sigmasecondpeak / sigma);
        hRawYieldsSigmaRatioSecondFirstPeak->SetBinError(
            iPt + 1, TMath::Sqrt(sigmaerr * sigmaerr / (sigma * sigma) +
                                 sigmasecondpeakerr * sigmasecondpeakerr /
                                     (sigmasecondpeak * sigmasecondpeak)) *
                         sigmasecondpeak /
                         sigma); // neglected correlation between parameters

        hRawYieldsSecondPeakTrue->SetBinContent(iPt + 1, rawyield);
        hRelDiffRawYieldsSecondPeakFitTrue->SetBinContent(iPt + 1, rawyield);
      }
      if (SgnFunc[iPt] == AliHFInvMassFittert::k2Gaus) {
      ...
      auto massFitter1 =
          new AliHFInvMassFittert(hMassForFit1[iPt], MassMin[iPt], MassMax[iPt],
                                  BkgFunc[iPt], SgnFunc[iPt]);
      auto massFitter2 =
          new AliHFInvMassFittert(hMassForFit2[iPt], MassMin[iPt], MassMax[iPt],
                                  BkgFunc[iPt], SgnFunc[iPt]);

      TPad *pad1 = new TPad("prompt", "", 0, 0., 1.0, 1.0);
      TPad *pad2 = new TPad("fd", "", 0.5, 0., 1, 1);

      if (degPol[iPt] > 0)
        massFitter1->SetPolDegreeForBackgroundFit(degPol[iPt]);
      if (UseLikelihood)
        massFitter1->SetUseLikelihoodFit();
      if (fixMean[iPt])
        massFitter1->SetFixGaussianMean(hMeanToFix->GetBinContent(iPt + 1));
      if (boundMean)
        massFitter1->SetBoundGaussianMean(massForFit, MassMin[iPt],
                                          MassMax[iPt]);
      else {
        massFitter1->SetInitialGaussianMean(massForFit);
        massFitter2->SetInitialGaussianMean(massForFit);
      }
      if (fixSigma[iPt]) {
        if (!isSigmaMultFromUnc)
          massFitter1->SetFixGaussianSigma(hSigmaToFix->GetBinContent(iPt + 1) *
                                           sigmaMult);
        else {
          if (sigmaMultFromUnc == "MinusUnc")
            massFitter1->SetFixGaussianSigma(
                hSigmaToFix->GetBinContent(iPt + 1) -
                hSigmaToFix->GetBinError(iPt + 1));
          else if (sigmaMultFromUnc == "PlusUnc")
            massFitter1->SetFixGaussianSigma(
                hSigmaToFix->GetBinContent(iPt + 1) +
                hSigmaToFix->GetBinError(iPt + 1));
          else
            cout << "WARNING: impossible to fix sigma! Wrong mult factor set "
                    "in config file!"
                 << endl;
        }
      } else {
        if (hSigmaToFix) {
          massFitter1->SetInitialGaussianSigma(
              hSigmaToFix->GetBinContent(iPt + 1) * sigmaMult);
          massFitter2->SetInitialGaussianSigma(
              hSigmaToFix->GetBinContent(iPt + 1) * sigmaMult);
        }

        else if (particle == kDstar)
          massFitter1->SetInitialGaussianSigma(0.001);
        else
          massFitter1->SetInitialGaussianSigma(0.008);
      }
      if (fixSigmaRatio) {
        massFitter1->SetFixRatio2GausSigma(
            hSigmaToFix1->GetBinContent(iPt + 1) /
            hSigmaToFix2->GetBinContent(iPt + 1));
        massFitter2->SetFixRatio2GausSigma(
            hSigmaToFix1->GetBinContent(iPt + 1) /
            hSigmaToFix2->GetBinContent(iPt + 1));
      }
      ...
      if (InclSecPeak[iPt] && particle == kDs) {
        if (hSigmaToFixSecPeak) {
          massFitter1->IncludeSecondGausPeak(
              massDplus, false,
              hSigmaToFixSecPeak->GetBinContent(iPt + 1) * sigmaMultSecPeak,
              true);
          if (fixSigmaToFirstPeak) {
            // fix D+ peak to sigmaMC(D+)/sigmaMC(Ds+)*sigmaData(Ds+)
            massFitter1->MassFitter(false);
            double sigmaFirstPeak = massFitter1->GetSigma();
            double sigmaRatioMC = hSigmaToFixSecPeak->GetBinContent(iPt + 1) /
                                  hSigmaFirstPeakMC->GetBinContent(iPt + 1);
            massFitter1->IncludeSecondGausPeak(
                massDplus, false, sigmaRatioMC * sigmaFirstPeak, true);
          }
        } else {
          massFitter1->IncludeSecondGausPeak(massDplus, false,
                                             SigmaSecPeak[iPt], true);
        }
      }

      if (enableRef) {
        double rOverS = hMassForSig1[iPt]->Integral(
            hMassForSig1[iPt]->FindBin(MassMin[iPt] * 1.0001),
            hMassForSig1[iPt]->FindBin(MassMax[iPt] * 0.999));
        rOverS = hMassForRel1[iPt]->Integral(
                     hMassForRel1[iPt]->FindBin(MassMin[iPt] * 1.0001),
                     hMassForRel1[iPt]->FindBin(MassMax[iPt] * 0.999)) /
                 rOverS;
        massFitter1->SetFixReflOverS(rOverS);
        massFitter1->SetTemplateReflections(hMassRef1[iPt], "2gaus",
                                            MassMin[iPt], MassMax[iPt]);

        massFitter2->SetFixReflOverS(rOverS);
        massFitter2->SetTemplateReflections(hMassRef2[iPt], "2gaus",
                                            MassMin[iPt], MassMax[iPt]);
      }

      massFitter1->MassFitter(false);
      massFitter2->MassFitter(false);

      double rawyield = massFitter1->GetRawYield();
      double rawyielderr = massFitter1->GetRawYieldError();
      double sigma = massFitter1->GetSigma();
      double sigmaerr = massFitter1->GetSigmaUncertainty();
      double mean = massFitter1->GetMean();
      double meanerr = massFitter1->GetMeanUncertainty();
      double redchi2 = massFitter1->GetReducedChiSquare();
      double signif = 0., signiferr = 0.;
      double sgn = 0., sgnerr = 0.;
      double bkg = 0., bkgerr = 0.;
      massFitter1->Significance(3, signif, signiferr);
      massFitter1->Signal(3, sgn, sgnerr);
      massFitter1->Background(3, bkg, bkgerr);
      ...
      hRawYieldsSoverB->SetBinError(
          iPt + 1, sgn / bkg *
                       TMath::Sqrt(sgnerr / sgn * sgnerr / sgn +
                                   bkgerr / bkg * bkgerr / bkg));
      ...
      for (int iS = 0; iS < nMassWindows; iS++) {
        massFitter1->Significance(nSigma4SandB[iS], signif, signiferr);
        massFitter1->Signal(nSigma4SandB[iS], sgn, sgnerr);
        massFitter1->Background(nSigma4SandB[iS], bkg, bkgerr);
      ...
      TF1 *fTotFunc = massFitter1->GetMassFunc();
      TF1 *fBkgFunc = massFitter1->GetBackgroundRecalcFunc();
      ...
      if (InclSecPeak[iPt] && particle == kDs) {
        ...
        double rawyieldSecPeak = fTotFunc->GetParameter(paryieldSecPeak) /
                                 hMassForFit1[iPt]->GetBinWidth(1);
        double rawyieldSecPeakerr = fTotFunc->GetParError(paryieldSecPeak) /
                                    hMassForFit1[iPt]->GetBinWidth(1);
        ...
        double bkgSecPeak =
            fBkgFunc->Integral(meansecondpeak - 3 * sigmasecondpeak,
                               meansecondpeak + 3 * sigmasecondpeak) /
            hMassForFit1[iPt]->GetBinWidth(1);
        double bkgSecPeakerr = TMath::Sqrt(bkgSecPeak);
        double signalSecPeak =
            fTotFunc->Integral(meansecondpeak - 3 * sigmasecondpeak,
                               meansecondpeak + 3 * sigmasecondpeak) /
                hMassForFit1[iPt]->GetBinWidth(1) -
            bkgSecPeak;
        double signalSecPeakerr = TMath::Sqrt(signalSecPeak + bkgSecPeak);
        double signifSecPeak = -1., signifSecPeakerr = -1.;
        AliHFInvMassFittert::ComputeSignificance(
            signalSecPeak, signalSecPeakerr, bkgSecPeak, bkgSecPeakerr,
        ...
        hRawYieldsSignificanceSecondPeak->SetBinError(iPt + 1,
                                                      signifSecPeakerr);
        hRawYieldsSigmaRatioSecondFirstPeak->SetBinContent(
            iPt + 1, sigmasecondpeak / sigma);
        hRawYieldsSigmaRatioSecondFirstPeak->SetBinError(
            iPt + 1, TMath::Sqrt(sigmaerr * sigmaerr / (sigma * sigma) +
                         ...
                         sigmasecondpeak /
                         sigma); // neglected correlation between parameters
        hRawYieldsSoverBSecondPeak->SetBinContent(iPt + 1,
                                                  signalSecPeak / bkgSecPeak);
        hRawYieldsSoverBSecondPeak->SetBinError(
            iPt + 1, signalSecPeak / bkgSecPeak *
                         TMath::Sqrt(signalSecPeakerr / signalSecPeak *
                                         signalSecPeakerr / signalSecPeak +
                                     bkgSecPeakerr / bkgSecPeak *
                                         bkgSecPeakerr / bkgSecPeak));
      ...
      massFitter2->DrawHere(pad1, 3, -1);
      //            massFitter1->DrawHere1(pad2,2,-1);
      ...
      if (!isMC) {
        ...
        if (nPtBins > 1)
          ...
          cResiduals[iCanv]->cd();
        massFitter1->DrawHistoMinusFit(gPad);
      }
    }
  ...
  //        {
  //            if(iCanv == 0 && nCanvases > 1)
  //                cResiduals[iCanv]->SaveAs(Form("%s[",
  //                outFileNameRes.Data()));
  ...
  //  outFileName.ReplaceAll(".root",".eps");
  //  TString outFileNameRes = outFileName;
  //  outFileNameRes.ReplaceAll(".eps", "_Residuals.eps");
  //  for(int iCanv=0; iCanv<nCanvases; iCanv++)
  //  {
  //      if(iCanv == 0 && nCanvases > 1)
  //          cMass[iCanv]->SaveAs(Form("%s[", outFileName.Data()));
  ...
  TString outFileNameRes = outFileName;
  outFileNameRes.ReplaceAll(".png", "_Residuals.png");
  for (int iCanv = 0; iCanv < nCanvases; iCanv++) {
    if (iCanv == 0 && nCanvases > 1)
      cMass[iCanv]->SaveAs(Form("%s[", outFileName.Data()));
    cMass[iCanv]->SaveAs(outFileName.Data());
...
//__________________________________________________________________________________________________________________
double DoubleGaus(double *m, double *pars) {
  double norm = pars[0], mean = pars[1], sigma1 = pars[2], sigma1_2 = pars[3],
         fg = pars[4];

  return norm * ((1 - fg) * TMath::Gaus(m[0], mean, sigma1, true) +
                 fg * TMath::Gaus(m[0], mean, sigma1_2, true));
}

//__________________________________________________________________________________________________________________
double DoubleGausSigmaRatio(double *m, double *pars) {
  double norm = pars[0], mean = pars[1], sigma1 = pars[2],
         sigma1OverSigma2 = pars[3], fg = pars[4];

  return norm * ((1 - fg) * TMath::Gaus(m[0], mean, sigma1, true) +
                 fg * TMath::Gaus(m[0], mean, sigma1 / sigma1OverSigma2, true));
}
...
double DoublePeakSingleGaus(double *m, double *pars) {
  ...
  return norm1 * TMath::Gaus(m[0], mean1, sigma1, true) +
         norm2 * TMath::Gaus(m[0], mean2, sigma2, true);
}

//__________________________________________________________________________________________________________________
double DoublePeakDoubleGaus(double *m, double *pars) {
  double norm1 = pars[0], mean = pars[1], sigma1 = pars[2], sigma1_2 = pars[3],
         fg = pars[4];                                       // Ds peak
  double norm2 = pars[5], mean2 = pars[6], sigma2 = pars[7]; // Dplus peak

  return norm1 * ((1 - fg) * TMath::Gaus(m[0], mean, sigma1, true) +
                  fg * TMath::Gaus(m[0], mean, sigma1_2, true)) +
         norm2 * TMath::Gaus(m[0], mean2, sigma2, true);
}
